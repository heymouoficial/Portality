...
const manageInfrastructureTool = {
...
const manageGoogleWorkspaceTool = {
...
export const openRouterService = {
    ...
    async streamChat(messages, userContext, onChunk, onToolCall) {
...
`.trim();
                    ...
                    if (name === 'query_knowledge_base') {
                        ...
                        if (args.service === 'gmail') {
                            ...
                            if (args.action === 'create_meeting') {
                                ...
                                const result = await googleService.createCalendarEvent(args.subject, args.body, startTime, endTime, args.to ? [args.to] : []);
                                toolResultString = JSON.stringify(result);
            ...
            if (error.message?.includes("API key"))
                friendlyError = "Falta la API Key en el entorno.";
            if (error.message?.includes("404"))
                friendlyError = "Modelo no disponible.";
            if (error.message?.includes("thought signature"))
                friendlyError = "Error de sincronización (Thought Signature). Reintentando...";

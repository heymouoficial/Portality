...
class NotionService {
    ...
    async createTask(task) {
        const dbId = import.meta.env.VITE_NOTION_DB_TASKS;
        if (!dbId)
            throw new Error('Missing Task DB ID');
    ...
    async createClient(clientData) {
        const dbId = import.meta.env.VITE_NOTION_DB_CLIENTS;
        if (!dbId)
            throw new Error('Missing Client DB ID');
        const properties = {
    ...
    mapClient(page) {
        ...
        return {
            ...
            type: props.Type?.select?.name || props['Tipo de servicio']?.select?.name || 'fixed',
            status: props.Status?.select?.name || props.Estado?.select?.name || 'active',
            notion_id: page.id
        };
    }
    mapService(page) {
        ...
        return {
            ...
            frequency: props.Frequency?.select?.name || props['Frecuencia']?.select?.name || 'monthly'
        };
    }
    mapTask(page) {
        const props = page.properties;
        ...
        let status = 'todo';
        if (['Done', 'Listo', 'Completada'].includes(rawStatus))
            status = 'done';
        if (['In Progress', 'En curso', 'En Progreso'].includes(rawStatus))
            status = 'in-progress';
        return {
            id: page.id,
            title: props.Name?.title[0]?.plain_text || props['Nombre de tarea']?.title[0]?.plain_text || 'Untitled',
            priority: props.Priority?.select?.name || props.Prioridad?.select?.name || 'medium',
            status: status,
    ...
    mapCalendarEvent(page) {
        ...
        return {
            ...
            type: props.Type?.select?.name || 'meeting'
        };
    }
    /**
     * Legacy/Support methods
     */
    async provisionWorkspace(organizationId, orgName) {
        // Kept as is for now
    ...
    async checkSyncStatus(organizationId) {
        try {

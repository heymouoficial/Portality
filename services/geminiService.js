...
class GeminiService {
    ...
    `;
            const contextParts = [];
            if (context.userName)
                contextParts.push(`Usuario: ${context.userName}`);
            if (context.pendingTasks !== undefined)
                contextParts.push(`Tareas pendientes: ${context.pendingTasks}`);
            if (context.currentView)
                contextParts.push(`Vista actual: ${context.currentView}`);
            if (context.ragContext) {
                contextParts.push(`\n[CONTEXTO INFORMACIÓN AGENCIA (RAG)]:\nUse esta información si es relevante:${context.ragContext}`);
            }
            if (context.integrations) {
                const intStatus = [];
                if (context.integrations.notion)
                    intStatus.push(`Notion: ${context.integrations.notion}`);
                if (context.integrations.hostinger)
                    intStatus.push(`Hostinger/VPS: ${context.integrations.hostinger}`);
                if (intStatus.length > 0)
                    contextParts.push(`Estado de Integraciones: ${intStatus.join(', ')}`);
            }
...
`;
        ...
        try {
            ...
            if (isQuotaError && retryCount < 3) {
                ...
                if (currentKey)
                    geminiKeyManager.reportError(currentKey);
...
`);
    ...
    async embedText(text) {
        ...
        try {
            ...
            if (!embeddingModel)
                throw new Error('Failed to get embedding model');
